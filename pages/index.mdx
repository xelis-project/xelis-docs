...
To helps its development, success, and provide continous support, we have set a decreasing dev fee curve starting at `15%` on block reward.  
...
The spending will be transparent, with all major expenses categorically detailed.  
...
There are multiple reasons why we opted for a decreasing fee curve, but the main point is to prevent big miners from earning 
...
As of now, XELIS does not have Smart Contract capabilities.
Slixe (the founder) will be the one with custody of thoses funds.
After the Smart Contract integration, we will be able to redirect the fees into an exclusive Smart Contract
with multiple trustees. It will allow to create a DAO and manage all of this in simplicity and trustlessly.
...
funds for the sole purpose of maintaining a healhty environment. We are the one who have the best interest for this platform, not powerful entities 
