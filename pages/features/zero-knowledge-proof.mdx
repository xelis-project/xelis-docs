...
**XELIS** uses zero-knowledge proof to prove that a ciphertext (transfered amount which is in encrypted form) in a transaction is valid.
To verify such claims, we do two verifications:
- First, we verify that the transfered amount is **never** above encrypted balance.
