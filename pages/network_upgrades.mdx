...
This is the first network upgrade, primarily aimed at adjusting the mining algorithm to be more egalitarian across devices
and enhance resistance against FPGAs.

The name of the new version of POW algorithm is [XelisHashV2](https://github.com/xelis-project/xelis-hash).
Additionally, during V0, we could not get an accurate estimate of the current hashrate, because
...
Another mandatory change has been introduced, increasing the extra data size limit per TX from 1 KB to 32 KB.
This allows more data to be integrated into a single transaction, such as a document or a picture.
Limit per transfer in a TX is set to 1 KB.

The difficulty algorithm has been adjusted to be less aggressive and more smooth in its adjustment.
