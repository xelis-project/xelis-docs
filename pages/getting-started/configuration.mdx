...
Boost sync mode is a faster way to sync the full blockchain. It is not enabled by default as it require more resources usage.
...
Ask peers to not share our IP to others and/or through API.<br/>
...
Set the filename to use for logging. By default, the filename is set to `xelis-daemon.log`.<br/>
...
Set the filename to use for logging. By default, the filename is set to `xelis-wallet.log`.<br/>
Every logs file are stored in the `logs` folder and are named with the date of the day.
