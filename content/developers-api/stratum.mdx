...
XELIS is using the [Xelis Hash](https://github.com/xelis-project/xelis-hash) as its Proof of Work algorithm.

Pool operators, please support the following algorithm names for compatibility with miners:
- `xel/v1` / `xel/0`: since block height `0`
- `xel/v2` / `xel/1`: since block height `434 100`
- `xel/v3` / `xel/2`: since block height `3 282 150`

The most recent algorithm is `xel/v3`.

The format `xel/v{version}` comes from the node protocol while `xel/{version}` comes from legacy stratum pools and/or miners.

Pool operators and miners developers, for backwards compatibility, you should support both formats.

The **primary** format is `xel/v{version}` for full compatibility with XELIS nodes.
...
Available error codes, in addition to the codes defined in the [JSON RPC 2.0](https://www.jsonrpc.org/specification) specification, are:
...
- `params` : `[ string, [ string ] ]`
  ...
  2. OPTIONAL, specifies a list of supported mining algorithms. If omitted, a default algorithm `xel/v3` or the most recent algorithm will be used.

Miners please use `xel/v3`

Pool operators, for backwards compatibility, you should also support `xel/1`.
Please see [PoW Algorithms](#pow-algorithms) for more details.
