...
```
**Gas Cost**: 1 lex  
**Description**: Prints a string representation of `message` (only effective if in debug mode).  
...
```
**Gas Cost**: 1 lex  
**Description**: Prints a debug-style (developer-friendly) representation of `message` (only effective if in debug mode).  
...
```
**Gas Cost**: 1 lex  
**Description**: Aborts execution with the provided `message` as a message. It never returns normally.  
...
```
**Gas Cost**: 1 lex  
**Description**: If `condition` is false, execution aborts with the provided `message`.  
...
```
**Gas Cost**: 250 lex  
**Description**: Increases the gas limit by `amount`. Returns `true` if successful.  
...
```
**Gas Cost**: 50 lex  
**Description**: Performs a bitwise XOR operation on two hashes and returns the result.  
...
```
**Gas Cost**: 2 lex  
**Description**: Appends `element` to the end of the array.  
...
```
**Gas Cost**: 1 lex  
**Description**: Removes and returns the last element if it exists, or `null` if the array is empty.  
...
```
**Gas Cost**: 10 lex  
**Description**: Returns true if `element` is in the array.  
...
```
**Gas Cost**: 5 lex  
**Description**: Inserts `element` at `index`, shifting subsequent elements to the right.  
...
```
**Gas Cost**: 5 lex  
**Description**: Returns the index of the first occurrence of `element` in the array, or `null` if not found.  
...
```
**Gas Cost**: 20 lex  
**Description**: Sorts the array in ascending order if `ascending` is true, or descending order if false.  
...
```
**Gas Cost**: 10 lex  
**Description**: Applies the `mapper` function to each element of the array and returns a new array containing the results.  
...
```
**Gas Cost**: 30 lex  
**Description**: Sorts the array based on the key returned by `key_fn`.  
...
```
**Gas Cost**: 1 lex  
**Description**: Returns the underlying value if not `null`. If `null`, the execution panics with the provided `message`.  
...
```
**Gas Cost**: 1 lex  
**Description**: Checks if `substring` is contained within the string.  
...
```
**Gas Cost**: 5 lex  
**Description**: Converts the string into an array of its UTF-8 bytes.  
...
```
**Gas Cost**: 3 lex  
**Description**: Finds the first index where `substring` occurs. If not found, returns `null`.  
...
```
**Gas Cost**: 3 lex  
**Description**: Finds the last index where `substring` occurs. If not found, returns `null`.  
...
```
**Gas Cost**: 5 lex  
**Description**: Returns a copy of the string with all occurrences of `from` replaced by `to`.  
...
```
**Gas Cost**: 3 lex  
**Description**: Returns true if the string begins with `prefix`.  
...
```
**Gas Cost**: 3 lex  
**Description**: Returns true if the calling string ends with `suffix`.  
...
```
**Gas Cost**: 5 lex  
**Description**: Checks if `element` is within `[start, end)`.  
...
```
**Gas Cost**: 7500 lex  
**Description**: Computes the SHA-3 hash of `input`.  
...
```
**Gas Cost**: 2500 lex  
**Description**: Creates a new asset with the given parameters. `max_supply` can be either `MaxSupplyMode::None`, `MaxSupplyMode::Fixed(u64)` , `MaxSupplyMode::Mintable(u64)`  . Returns the newly created asset or `null` if creation fails.  
