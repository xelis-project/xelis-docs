...
After obtaining a `Block` (e.g., <code>let blk = Block::current();)</code>, you can call <signature>blk.nonce() ⟶ u64</signature>  
...
```
**Gas Cost**: 500 lex  
**Description**: Looks up the asset by its `Hash`. Returns the asset or `null` if not found.<br/>Once you have an `Asset`, you can call <signature>asset.get_max_supply() ⟶ optional<u64></signature>:  
