...
The [`silex` CLI](https://github.com/xelis-project/silex-cli) is the command-line toolset for working with Silex programs and compiled modules. It supports the development workflow from source code to bytecode and inspection:

- **Compile** `.slx` source files into compiled `.slxc` modules.
- **Run** source files or compiled modules to execute a program.
- **Generate ABIs** from Silex source for tooling and integrations.
- **Assemble** textual bytecode into a compiled module.
- **Disassemble** a compiled module into readable assembly for inspection.

Use it to compile and run contracts during development, or inspect the bytecode produced for deployment. See the [CLI repository](https://github.com/xelis-project/silex-cli) for installation instructions and command options.
...
Every Silex contract can define several *entry* functions. These are the callable entry points used by external invocations.
...
Marked by `entry`; they are the main entry points for external calls to the contract.

When an entry has no explicit return type, it uses the default `u64` exit code. Return `0` for success, or any value greater than `0` to report a contract-defined error code. This is useful when a contract wants callers and indexers to distinguish exact failure reasons without parsing text.
...
If you add an explicit return type, the entry returns data instead of only the default exit code. The encoded return value can be up to 256 bytes and is stored as the execution `exit_value` in the contract logs.

```rust
entry greeting(name: string) -> string {
    return "Hello, " + name
}
```

If execution raises an error, for example because `require(condition, "message")` fails, the error is also stored in the contract logs.

```rust
entry checked(value: u64) {
...
Marked by `hook`; these are system-invoked functions with special lifecycle behavior.

Currently, `constructor()` is the only hook. It is called once during contract deployment, before the contract is available for normal entry invocations. After deployment finishes, the constructor cannot be called again by users, other contracts, entries, or the contract itself.

Use `constructor()` for one-time setup such as initializing storage, recording the owner, or preparing configuration needed by later entries. It must explicitly return `u64`; `0` indicates success, and any value greater than `0` can be used as a deployment-time error code. If the contract defines a constructor, the deployment transaction must include an `invoke` payload with a `max_gas` limit so the constructor can execute.
...
- The `entry main` function is the main entry point and stores a typed `u64` exit value.
